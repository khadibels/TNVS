...
function findInvite(PDO $pdo, string $raw) {
  ...
  if ($inv['token_expires_at'] && strtotime($inv['token_expires_at']) < time()) throw new Exception('Link expired.');
  if (in_array($inv['status'], ['awarded','cancelled'], true)) throw new Exception('RFQ closed.');
  return $inv;
}
function rfqItemQtyMap(PDO $pdo, int $rfq_id): array {
  ...
  $st->execute(); $cols=array_flip($st->fetchAll(PDO::FETCH_COLUMN,0));
  $qtyCol = isset($cols['qty'])?'qty':(isset($cols['quantity'])?'quantity':null);
  if ($qtyCol) {
    $s = $pdo->prepare("SELECT id, $qtyCol AS qty FROM rfq_items WHERE rfq_id=?");
  } else {
    $s = $pdo->prepare("SELECT id, 1 AS qty FROM rfq_items WHERE rfq_id=?");
  }
  $s->execute([$rfq_id]); $map=[];
  while($r=$s->fetch(PDO::FETCH_ASSOC)) $map[(int)$r['id']]=(float)$r['qty'];
  return $map;
}
...
try {
  ...
  $raw = $_POST['t'] ?? '';
  if (!preg_match('/^[a-f0-9]{64}$/', $raw)) throw new Exception('Bad token');
  ...
  foreach ($prices as $rfq_item_id => $p) {
    ...
    if ($rfq_item_id > 0 && $val !== null) $clean[$rfq_item_id] = $val;
  }

  $lead = isset($_POST['lead_time_days']) ? max(0, (int)$_POST['lead_time_days']) : 0;
  ...
  if ($clean) {
    $ins = $pdo->prepare("INSERT INTO quote_items (quote_id, rfq_item_id, unit_price, currency)
                          VALUES (?, ?, ?, 'PHP')");
    foreach ($clean as $iid => $unit) $ins->execute([$quote_id, $iid, $unit]);
  }

  // attachments
  if (!empty($_FILES['files']) && is_array($_FILES['files']['name'])) {
    $dir = __DIR__ . '/../../uploads/quotes/' . date('Y/m') . "/rfq_{$rfq_id}/supplier_{$supplier_id}";
    ...
    for ($i=0; $i<count($_FILES['files']['name']); $i++) {
      ...
      if ($size > 20*1024*1024) continue; // 20 MB
      $safe = preg_replace('/[^a-zA-Z0-9._-]/','_', $name);
      $dest = $dir . '/' . uniqid('f_', true) . '_' . $safe;
      if (@move_uploaded_file($tmp, $dest)) {
        $rel = str_replace(realpath(__DIR__.'/../../'), '', realpath($dest));
        $insA->execute([$quote_id, $name, $rel, $mime, $size]);
      }
    }
  }

  // Optionally flip RFQ to 'sent' if still draft
  $pdo->prepare("UPDATE rfqs SET status = CASE WHEN status='draft' THEN 'sent' ELSE status END WHERE id=?")->execute([$rfq_id]);
  ...
  echo "<div style='font-family:system-ui;padding:2rem;color:#b00'><b>Failed:</b> ".htmlspecialchars($e->getMessage())."</div>";
}
