...
function findInvite(PDO $pdo, string $raw): array {
  if (!preg_match('/^[a-f0-9]{32,64}$/i', $raw)) throw new Exception('Bad token');
  $st = $pdo->prepare("SELECT rr.*, r.due_date, r.status
...
function rfqItemQtyMap(PDO $pdo, int $rfq_id): array {
  ...
  $sql = $qtyCol
    ...
    : "SELECT id, 1 AS qty FROM rfq_items WHERE rfq_id=?";
...
try {
  ...
  $total = 0.0; foreach ($clean as $iid => $unit) { $qty = $qtyMap[$iid] ?? 1; $total += $qty * $unit; }

  $pdo->beginTransaction();

  $stmt = $pdo->prepare("INSERT INTO quotes
      (rfq_id, supplier_id, submitted_at, lead_time_days, notes, total_cache, is_final)
      VALUES (?, ?, NOW(), ?, ?, ?, 1)");
  ...
  if (!empty($_FILES['files']) && is_array($_FILES['files']['name'])) {
    ...
    $insA = $pdo->prepare("INSERT INTO quote_attachments
        (quote_id, original_name, path, mime, size_bytes) VALUES (?, ?, ?, ?, ?)");
  ...
  $pdo->prepare("UPDATE rfqs
                 SET status = CASE WHEN status='draft' THEN 'sent' ELSE status END
                 WHERE id=?")->execute([$rfq_id]);
