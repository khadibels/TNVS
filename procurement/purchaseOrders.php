...
window.openEdit = async (id)=>{
  ...
  try {
    const j = await fetchJSON(api.get + '?id=' + id);  // <--- use new endpoint
    ...
    if (!j.items.length) addItemRow();  // always at least one
    recalcAll();
...
document.getElementById('poForm').addEventListener('submit', async (ev)=>{
  ...
  // auto-fill descr if qty/price entered but descr blank
