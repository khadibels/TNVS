...
try {
    ...
    if (!$items) {
        bad("At least one valid item (descr, qty>0, price>=0) is required");
    }
    ...
    if ($id > 0) {
        ...
        if (
            in_array($cur["status"], ["received", "closed", "cancelled"], true)
        ) {
            $pdo->rollBack();
            bad("Cannot edit a " . $cur["status"] . " PO", 409);
        }
    }
