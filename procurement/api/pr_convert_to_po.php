...
try {
    ...
    if (!$pr) {
        bad("pr_not_found", 404);
    }
    if (strtolower((string) $pr["status"]) !== "approved") {
        bad("pr_must_be_approved");
    }

    /* Load PR items */
    $it = $pdo->prepare(
        "SELECT descr, qty, price FROM procurement_request_items WHERE pr_id=?"
    );
    $it->execute([$id]);
    ...
    if (has_col($pdo, "purchase_orders", "supplier_id")) {
      ...
      "
            ...
            if (!$s) {
                $s = $pdo
                    ->query("SELECT id FROM suppliers ORDER BY id ASC LIMIT 1")
                    ->fetchColumn();
            }
            if (!$s) {
    ...
    foreach (["po_no", "status", "total"] as $m) {
        if (!has_col($pdo, $poTable, $m)) {
            bad("$poTable.$m missing");
        }
    }
    ...
    if (!has_col($pdo, $itemsTable, "po_id")) {
        bad("$itemsTable.po_id missing");
    }
    if (!has_col($pdo, $itemsTable, "descr")) {
        bad("$itemsTable.descr missing");
    }
    if (!has_col($pdo, $itemsTable, "qty")) {
        bad("$itemsTable.qty missing");
    }
    if (!has_col($pdo, $itemsTable, "price")) {
        bad("$itemsTable.price missing");
    }

    $hasLineTotal = has_col($pdo, $itemsTable, "line_total");
    $insItem = $hasLineTotal
        ? $pdo->prepare(
            "INSERT INTO {$itemsTable} (po_id, descr, qty, price, line_total) VALUES (?,?,?,?,?)"
        )
        : $pdo->prepare(
            "INSERT INTO {$itemsTable} (po_id, descr, qty, price) VALUES (?,?,?,?)"
        );
    ...
    /* Update PO total */
    $pdo->prepare("UPDATE {$poTable} SET total=? WHERE id=?")->execute([
        $total,
        $po_id,
    ]);
