...
try {
  ...
  $allowedAll = ['draft','approved','ordered','partially_received','received','closed','cancelled'];
  if (!in_array($new, $allowedAll, true)) bad('invalid status');
  ...
  if (!in_array($new, $allowed[$cur] ?? [], true)) {
    $pdo->rollBack(); bad("Cannot change status from $cur to $new", 409);
  }
