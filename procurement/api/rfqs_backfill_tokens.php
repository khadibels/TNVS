...
try {
  ...
  $st = $pdo->query("SELECT id FROM rfq_recipients WHERE $tokCol IS NULL OR $tokCol = '' FOR UPDATE");
  $ids = $st->fetchAll(PDO::FETCH_COLUMN, 0);

  $upd = $pdo->prepare("UPDATE rfq_recipients SET $tokCol = ? WHERE id=?");
  $n=0;
  foreach ($ids as $id) {
    $token = hash('sha256', $id.'-'.bin2hex(random_bytes(16)).'-'.microtime(true));
    $upd->execute([$token, $id]);
    $n++;
  }
  $pdo->commit();

  echo json_encode(['ok'=>true, 'updated'=>$n, 'token_column'=>$tokCol]);
} catch (Throwable $e) {
