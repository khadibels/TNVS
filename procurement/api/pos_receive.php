...
try{
  ...
  if ($po_id<=0) bad('po_id required');

  // Expect: items[<po_item_id>][qty] = number to receive now
  $items = $_POST['items'] ?? [];
  if (!is_array($items) || !$items) bad('no items passed');
  ...
  $st->execute([$po_id]);
  $cur = $st->fetchColumn();
  if ($cur===false){ $pdo->rollBack(); bad('PO not found',404); }
  $cur = strtolower((string)$cur);
  if (in_array($cur, ['closed','cancelled'], true)) { $pdo->rollBack(); bad("PO is $cur",409); }
  ...
  if (!$lines){ $pdo->rollBack(); bad('No PO items',409); }

  $byId = [];
  foreach($lines as $r){ $byId[(int)$r['id']] = $r; }
  ...
  if ($applied===0){ $pdo->rollBack(); bad('No valid receive lines'); }
  ...
  $newStatus = ((float)$tot['r'] >= (float)$tot['q']) ? 'received' : 'partially_received';

  $pdo->prepare("UPDATE purchase_orders SET status=? WHERE id=?")->execute([$newStatus, $po_id]);

  $pdo->commit();
  echo json_encode(['ok'=>true, 'status'=>$newStatus, 'lines_updated'=>$applied]);
}catch(Throwable $e){
