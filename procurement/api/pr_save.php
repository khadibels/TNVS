...
function next_pr_no(PDO $pdo): string {
  // Simple generator: PR-YYYYMMDD-#### ; you can replace with counters table if you like
  $base = 'PR-'.date('Ymd').'-';
...
try {
  ...
  $department_id = $_POST['department_id'] === '' ? null : (int)($_POST['department_id'] ?? 0);
  $status        = strtolower(trim((string)($_POST['status'] ?? 'draft')));
  $notes         = trim((string)($_POST['notes'] ?? ''));

  if ($title === '') bad('title_required');
  $allowed = ['draft','submitted','approved','rejected','fulfilled','cancelled'];
  if (!in_array($status, $allowed, true)) bad('invalid_status');
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $q = (float)($qtys[$i]   ?? 0);
    $p = (float)($prices[$i] ?? 0);
    // keep rows that have at least qty or price or description
    if ($d !== '' || $q > 0 || $p > 0) {
      if ($d === '') $d = 'Item '.($i+1);
      $items[] = ['descr'=>$d, 'qty'=>$q, 'price'=>$p];
    }
  }
  if (!$items) bad('items_required');

  // if updating, block edits unless current status allows
  if ($id) {
    ...
    // Allow edits only when draft or submitted (change to only 'draft' if you want stricter)
  ...
  // replace items (simplest & reliable)
  ...
  if (has_col($pdo,'procurement_requests','estimated_total')) {
    $pdo->prepare("UPDATE procurement_requests SET estimated_total=? WHERE id=?")
        ->execute([$total, $id]);
  }

  $pdo->commit();

  // return pr_no for convenience
  $pr_no = null;
  $st = $pdo->prepare("SELECT pr_no FROM procurement_requests WHERE id=?");
  $st->execute([$id]);
  $pr_no = $st->fetchColumn();

  echo json_encode(['ok'=>true, 'id'=>$id, 'pr_no'=>$pr_no, 'estimated_total'=>$total]);
} catch (Throwable $e) {
