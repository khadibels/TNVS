...
function next_seq_atomic(PDO $pdo, string $day): int
{
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            "UPDATE rfq_counters SET seq = LAST_INSERT_ID(seq + 1) WHERE day = ?"
        );
        $stmt->execute([$day]);
...
try {
    ...
    if ($id) {
        // UPDATE
        $sql =
            "UPDATE rfqs SET title=:title, due_date=:due_date, status=:status, notes=:notes WHERE id=:id";
    ...
    if (isset($_POST["suppliers"])) {
        ...
        if ($sel) {
            ...
            $st = $pdo->prepare(
                "SELECT id, COALESCE(email,'') AS email FROM suppliers WHERE id IN ($in)"
            );
            $st->execute($sel);
            while ($r = $st->fetch(PDO::FETCH_ASSOC)) {
                $emails[(int) $r["id"]] = $r["email"];
            }
        }
