...
function tokenCol(PDO $pdo): string {
  ...
  $st->execute(); $col=$st->fetchColumn();
  if(!$col) throw new Exception("Token column not found on rfq_recipients");
  return $col;
}
function ensureToken(PDO $pdo, int $rfq_id, int $supplier_id, string $col): string {
  $sel=$pdo->prepare("SELECT `$col` FROM rfq_recipients WHERE rfq_id=? AND supplier_id=?");
  $sel->execute([$rfq_id,$supplier_id]); $tok=$sel->fetchColumn();
  if ($tok && preg_match('/^[a-f0-9]{64}$/',$tok)) return $tok;
  $tok = bin2hex(random_bytes(32)); // 64 hex
  $exp = (new DateTime('+7 days'))->format('Y-m-d H:i:s');
  $up=$pdo->prepare("UPDATE rfq_recipients SET `$col`=?, token_expires_at=?, sent_at=NOW() WHERE rfq_id=? AND supplier_id=?");
...
try {
  ...
  if (!$base) {
    // fallback for localhost if BASE_URL isn’t set
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS']!=='off') ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
    $prefix = dirname(dirname($_SERVER['REQUEST_URI'] ?? '/procurement/api/')) ?: '';
  ...
  foreach ($rows as $r) {
    ...
    $mailto = "mailto:".rawurlencode($r['email'])
            ."?subject=".rawurlencode("Invitation to Quote — RFQ #$rfq_id")
            ."&body=".rawurlencode("Hi {$r['name']},\n\nPlease submit your quotation here:\n$link\n\nThanks!");
    $out[] = [
