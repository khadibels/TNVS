...
try {
  ...
  if (!empty($_POST['delete']) && isset($_POST['id'])) {
    ...
    echo json_encode(['ok'=>1,'deleted'=>$id]); exit;
  }
  ...
  if ($id>0) {
    $st=$pdo->prepare("UPDATE departments SET name=?, is_active=? WHERE id=?");
    $st->execute([$name,$active,$id]);
    echo json_encode(['ok'=>1,'id'=>$id,'mode'=>'update']);
  } else {
    $st=$pdo->prepare("INSERT INTO departments (name,is_active) VALUES (?,?)");
    $st->execute([$name,$active]);
    echo json_encode(['ok'=>1,'id'=>$pdo->lastInsertId(),'mode'=>'insert']);
  }
} catch(Throwable $e) {
