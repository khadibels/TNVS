...
if ($month!==null && ($month<1 || $month>12)) { http_response_code(400); echo json_encode(['error'=>'Invalid month']); exit; }

if ($id) {
  $st=$pdo->prepare("UPDATE budgets SET fiscal_year=?, month=?, department_id=?, category_id=?, amount=?, notes=?, updated_at=NOW() WHERE id=?");
  $st->execute([$year,$month,$dept,$cat,$amount,$notes,$id]);
  echo json_encode(['ok'=>1,'id'=>$id]); exit;
} else {
  $st=$pdo->prepare("INSERT INTO budgets (fiscal_year,month,department_id,category_id,amount,notes) VALUES (?,?,?,?,?,?)");
  $st->execute([$year,$month,$dept,$cat,$amount,$notes]);
  echo json_encode(['ok'=>1,'id'=>(int)$pdo->lastInsertId()]); exit;
}
