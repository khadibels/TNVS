...
/* total count */
$sqlCount = "SELECT COUNT(*) FROM vendors {$whereSql}";
...
$uploadWebPath = rtrim($base . '/vendor_portal/vendor/uploads/', '/') . '/';
$uploadFsPath  = rtrim(realpath(__DIR__ . '/../../vendor_portal/vendor/uploads/'), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;

$out = array_map(function($r) use ($uploadWebPath, $uploadFsPath) {
    $photo_url = null;
    if (!empty($r['profile_photo'])) {
        $name = basename($r['profile_photo']);     
        $file = $uploadFsPath . $name;
        if (is_file($file)) {
            $photo_url = $uploadWebPath . rawurlencode($name);
        }
    }
    return [
        'id'             => (int)$r['id'],
