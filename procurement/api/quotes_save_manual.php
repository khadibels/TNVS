<?php
// procurement/api/quotes_save_manual.php
...
try{
  ...
  $st = $pdo->prepare("
    ...
    VALUES (?, ?, ?, ?, ?, NOW(), 0)
  ");
  $st->execute([$rfq_id,$supplier_id, $lead_time_days ?: null, $notes, $is_final ? 1 : 0]);
  ...
  if ($items){
    ...
    foreach($items as $it){
      ...
      $ins->execute([$quote_id, $it['descr'], $it['qty'], $it['price'], $line]);
      $total += $line;
    }
  } else {
  ...
  $pdo->prepare("UPDATE quotes SET total_amount=? WHERE id=?")->execute([round($total,2),$quote_id]);

  $pdo->commit();
  echo json_encode(['ok'=>true, 'quote_id'=>$quote_id, 'total'=>round($total,2)]);
}catch(Throwable $e){
