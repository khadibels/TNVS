...
<?php if ($section === 'procurement') { ?>
  <div class="sidebar d-flex flex-column">
    ...
    <nav class="nav flex-column px-2 mb-4">
      ...
      <!-- FIX: use 'budgets' key to match pages setting $active='budgets' -->
...
<?php if ($role === 'admin') { ?>
  <div class="sidebar d-flex flex-column">
    ...
    <nav class="nav flex-column px-2 mb-4">
      ...
      <a class="nav-link d-flex align-items-center justify-content-between settings-parent<?= $isSettingsOpen ? ' active is-open' : '' ?>"
         data-bs-toggle="collapse" data-bs-target="#adminSettings" href="#" role="button"
         aria-expanded="<?= $isSettingsOpen ? 'true' : 'false' ?>" aria-controls="adminSettings">
