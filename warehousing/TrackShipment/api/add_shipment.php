...
if (strtotime($eta) < strtotime($pickup)) {
    ...
    echo json_encode([
        "ok" => false,
        "err" => "ETA Delivery cannot be before Pickup Date",
    ]);
    exit();
}
...
if (!$origin_id || !$dest_id) {
    ...
    echo json_encode([
        "ok" => false,
        "err" => "Origin and Destination are required",
    ]);
    exit();
}
if ($origin_id === $dest_id) {
    ...
    echo json_encode([
        "ok" => false,
        "err" => "Origin and Destination must be different",
    ]);
    exit();
}
...
try {
    ...
    $ev = $pdo->prepare(
        "INSERT INTO shipment_events (shipment_id, event_type, details, user_id) VALUES (?,?,?,?)"
    );
    $ev->execute([
        $sid,
        "Draft",
...
if (
    $expected_pickup &&
    $expected_delivery &&
    $expected_delivery < $expected_pickup
) {
    http_response_code(400);
