...
if ($carrier!== '') { $w[] = 's.carrier = ?'; $p[] = $carrier; }
...
$tot = [
  'total'=>0,'delivered'=>0,'in_transit'=>0,'delayed'=>0,'cancelled'=>0,'returned'=>0,
  'on_time_rate'=>0,'avg_transit_days'=>null
];
$byStatus = ['Draft'=>0,'Ready'=>0,'Dispatched'=>0,'In Transit'=>0,'Delivered'=>0,'Delayed'=>0,'Cancelled'=>0,'Returned'=>0];
$carriers = [];  // carrier => ['total'=>..,'deliv'=>..,'ontime'=>..]
$lanes = [];     // "WH1 - Main → WH2 - Overflow" => count
$lateList = [];  // top late not-delivered

$sumTransit = 0; $cntTransit = 0;
$ontime = 0; $delivCnt = 0;

foreach ($rows as $r) {
  $tot['total']++;
  $stName = $r['status'] ?? '';
  if (isset($byStatus[$stName])) $byStatus[$stName]++;

  // delivered metrics
  $deliveredAt = $r['delivered_at'] ? new DateTimeImmutable($r['delivered_at']) : null;
  if ($stName === 'Delivered') {
    $tot['delivered']++; $delivCnt++;
    if ($deliveredAt) {
      $exp = $r['expected_delivery'] ? new DateTimeImmutable($r['expected_delivery']) : null;
      if ($exp && $deliveredAt <= $exp->setTime(23,59,59)) $ontime++;
    }
    ...
    if ($r['expected_delivery']) {
      ...
      if ($today > $exp) {
        ...
        $lateList[] = ['id'=>$r['id'],'ref_no'=>$r['ref_no'],'days_overdue'=>$days,'dest'=>$r['destination']];
      }
    }
    if ($stName === 'In Transit' || $stName === 'Dispatched' || $stName === 'Ready') {
      $tot['in_transit']++;
    }
  }

  // transit time = Delivered - first Dispatched (if both exist)
  $firstDisp = $r['first_dispatch'] ? new DateTimeImmutable($r['first_dispatch']) : null;
  ...
  // carriers
  $c = $r['carrier'] ?: '—';
  $carriers[$c] = $carriers[$c] ?? ['total'=>0,'deliv'=>0,'ontime'=>0];
  $carriers[$c]['total']++;
  if ($stName==='Delivered') {
    $carriers[$c]['deliv']++;
    if ($deliveredAt && $r['expected_delivery'] && $deliveredAt <= (new DateTimeImmutable($r['expected_delivery']))->setTime(23,59,59)) {
      $carriers[$c]['ontime']++;
    }
  }
...
$tot['avg_transit_days'] = $cntTransit ? round($sumTransit/$cntTransit, 1) : null;

// format carriers & lanes
$carOut = [];
foreach ($carriers as $name=>$m) {
  $rate = $m['deliv'] ? round($m['ontime']*100/$m['deliv'],1) : 0.0;
  $carOut[] = ['carrier'=>$name,'total'=>$m['total'],'delivered'=>$m['deliv'],'on_time_rate'=>$rate];
}
usort($carOut, fn($a,$b)=> $b['total'] <=> $a['total']);
$laneOut = [];
foreach ($lanes as $k=>$v) $laneOut[] = ['lane'=>$k,'total'=>$v];
...
if (isset($_GET['format']) && strtolower($_GET['format']) === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=shipments_report_' . date('Ymd_His') . '.csv');
    ...
    fputcsv($out, ['From', ($_GET['from'] ?? ''), 'To', ($_GET['to'] ?? ''), 'Status', ($_GET['status'] ?? ''), 'Carrier', ($_GET['carrier'] ?? '')]);
    ...
    foreach (($payload['late'] ?? []) as $r) {
        fputcsv($out, [$r['ref_no'] ?? '—', $r['dest'] ?? '—', $r['days_overdue'] ?? 0]);
    }
