...
function sync_item_total(PDO $pdo, int $itemId): void {
  $stmt = $pdo->prepare("SELECT COALESCE(SUM(qty),0) FROM stock_levels WHERE item_id = ?");
  $stmt->execute([$itemId]);
...
function get_level(PDO $pdo, int $itemId, int $locId): int {
  $stmt = $pdo->prepare("SELECT qty FROM stock_levels WHERE item_id = ? AND location_id = ?");
 ...
 */
function log_tx(PDO $pdo, int $itemId, ?int $from, ?int $to, int $qty, string $action, ?string $note, ?int $userId): void {
  $stmt = $pdo->prepare("
 ...
 * STOCK IN
 */
function stock_in(PDO $pdo, int $itemId, int $locId, int $qty, ?string $note, ?int $userId): void {
  ...
  try {
    ...
    // Lock then update
    $pdo->prepare("SELECT qty FROM stock_levels WHERE item_id = ? AND location_id = ? FOR UPDATE")
        ->execute([$itemId, $locId]);
    $pdo->prepare("UPDATE stock_levels SET qty = qty + ? WHERE item_id = ? AND location_id = ?")
 ...
 * STOCK OUT
 */
function stock_out(PDO $pdo, int $itemId, int $locId, int $qty, ?string $note, ?int $userId): void {
  if ($qty <= 0) throw new InvalidArgumentException('Quantity must be > 0');
  $pdo->beginTransaction();
  try {
    ensure_level_row($pdo, $itemId, $locId);
    ...
    $stmt = $pdo->prepare("SELECT qty FROM stock_levels WHERE item_id = ? AND location_id = ? FOR UPDATE");
    $stmt->execute([$itemId, $locId]);
    $cur = (int)($stmt->fetchColumn() ?? 0);
    if ($cur < $qty) throw new RuntimeException('Not enough stock in From location');
 ...
 * TRANSFER
 */
function stock_transfer(PDO $pdo, int $itemId, int $fromLoc, int $toLoc, int $qty, ?string $note, ?int $userId): void {
  if ($qty <= 0) throw new InvalidArgumentException('Quantity must be > 0');
  if ($fromLoc === $toLoc) throw new InvalidArgumentException('From and To must be different');
  $pdo->beginTransaction();
  try {
    ...
    log_tx($pdo, $itemId, $fromLoc, $toLoc, $qty, 'TRANSFER', $note, $userId);
