<?php
// Start session BEFORE any auth or output
...
// (Debug only – comment these 3 lines out on prod)
ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);

// Show admin controls while you build
$isAdmin = true;

/* ------------------------------------------------------------------
   DB: ensure base table + add columns in a portable way (MySQL 5.7+ / MariaDB)
------------------------------------------------------------------- */
...
foreach ($maybeCols as $def) { safe_add_column($pdo, 'assets', $def); }
    ...
    if ($name !== '') {
      try {
        ...
        $stmt->execute([
          ...
          ':installed_on'=>$deployment_date ?: null, // backwards compat
          ':gps_imei'=>$gps_imei !== '' ? $gps_imei : null,
          ':notes'=>$notes !== '' ? $notes : null,
        ]);
        $newId = (int)$pdo->lastInsertId();
        header('Location: '.$_SERVER['PHP_SELF'].'?highlight_id='.$newId);
        exit;
      } catch (PDOException $e) {
        $err = ($e->getCode() === '23000') ? 'Duplicate unique ID' : 'DB error';
  ...
  if ($op === 'update') {
    ...
    if ($id > 0) {
      ...
      try {
        ...
        header('Location: '.$_SERVER['PHP_SELF'].'?highlight_id='.$id);
        exit;
      } catch (PDOException $e) {
        $err = ($e->getCode() === '23000') ? 'Duplicate unique ID' : 'DB error';
        header('Location: '.$_SERVER['PHP_SELF'].'?err='.urlencode($err));
...
<head>
  ...
  <style>
    :root{
      --bg:#f5f7fb; --card:#fff; --accent:#6d28d9; --muted:#6b7280; --text:#111827;
      --success:#10b981; --danger:#ef4444; --warning:#f59e0b; --info:#3b82f6;
      --shadow: 0 10px 30px rgba(16,24,40,0.08);
    }
    body{background:linear-gradient(180deg,#f7f9fc 0%,var(--bg) 100%);color:var(--text)}
    ...
    .sticky-th { position: sticky; top: 0; background: linear-gradient(180deg,#fbfdff,#f7f9fc); z-index: 1; }
...
<body>
  <div class="container-fluid p-0">
    <div class="row g-0">
      ...
      <!-- Sidebar (styled like shipmentTracking) -->
      <div class="sidebar d-flex flex-column">
        ...
        <nav class="nav flex-column px-2 mb-4">
          ...
          <a class="nav-link" href="./mainReq.php">
            <ion-icon name="build-outline"></ion-icon><span>Maintenance Requests</span>
          </a>
          ...
          <div class="card-body">
            <form class="row g-2 align-items-end">
              ...
              <div class="col-12 col-md-1 d-grid">
                <button type="button" class="btn btn-outline-secondary" onclick="applyFilters()">Filter</button>
              </div>
            </form>
          </div>
        </section>
...
<section class="card shadow-sm mb-3">
  <div class="card-body">
    <?php if ($isAdmin): ?>
      <!-- IMPORTANT: real <form> with id, method POST, and action to this page -->
      <form id="addAssetForm" method="POST" action="<?= h($_SERVER['PHP_SELF']) ?>" class="row g-2">
        ...
        <div class="col-12 col-md-3">
          <input class="form-control" name="unique_id" placeholder="Unique ID (QR/RFID/Serial)">
        </div>
      ...
      <!-- tiny guard: tells you immediately if the form is missing in the DOM -->
      <script>
        (function(){
          if(!document.getElementById('addAssetForm')){
            console.error('Add Asset form not found — check for an extra </form> earlier on the page.');
          }
        })();
      </script>
    <?php else: ?>
                      ...
                      <div class="d-flex flex-wrap gap-1">
                        <?php if ($isAdmin): ?>
                          ...
                          <?php if ($st==='Deployed'): ?>
                            ...
                            <form method="POST" class="d-inline" onsubmit="return confirm('Retire this asset?')">
                              <input type="hidden" name="op" value="transition">
                              <input type="hidden" name="csrf" value="<?= h($csrf) ?>">
                              <input type="hidden" name="id" value="<?= (int)$a['id'] ?>">
                          ...
                          <?php endif; ?>
                          <?php if ($st==='Retired'): ?>
                            <form method="POST" class="d-inline" onsubmit="return confirm('Mark this asset as disposed?')">
                              <input type="hidden" name="op" value="transition">
                              <input type="hidden" name="csrf" value="<?= h($csrf) ?>">
                              <input type="hidden" name="id" value="<?= (int)$a['id'] ?>">
  ...
  <script>
    ...
    (function(){
      var el=document.querySelector('tr.highlight');
