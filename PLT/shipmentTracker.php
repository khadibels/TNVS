<?php
// /plt/shipmentTracker.php  (role-free)
      ...
      <tr>
        ...
        <td class="text-end">
          ...
          <div class="btn-group btn-group-sm">
            ${r.status!=='picked'     ? `<button class="btn btn-outline-info"     onclick="setStatus(${r.id},'picked')">Picked</button>`:''}
            ${r.status!=='in_transit' ? `<button class="btn btn-outline-primary"  onclick="setStatus(${r.id},'in_transit')">In-Transit</button>`:''}
            ${r.status!=='delivered'  ? `<button class="btn btn-success"          onclick="setStatus(${r.id},'delivered')">Delivered</button>`:''}
            ${r.status!=='cancelled'  ? `<button class="btn btn-outline-danger"   onclick="setStatus(${r.id},'cancelled')">Cancel</button>`:''}
          </div>
        </td>
      </tr>`;
...
window.go=(p)=>{ if(!p||p<1) return; state.page=p; loadShipments().catch(e=>alert(parseErr(e))); };
...
document.getElementById('btnFilter').addEventListener('click', ()=>{
  ...
  loadShipments().catch(e=>alert(parseErr(e)));
});
