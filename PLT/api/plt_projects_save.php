...
try {
  if (!isset($pdo)) jerr('DB missing', 500);
  ...
  // Normalize empties to NULL (so we don’t need SQL NULLIF())
  $code  = ($code  === '') ? null : $code;
  ...
  if ($id > 0) {
    ...
    $sql = 'UPDATE plt_projects
             ...
             WHERE id = ?';
    $pdo->prepare($sql)->execute([$code, $name, $scope, $start, $dead, $status, $owner, $id]);
  } else {
    // Auto-generate a code if missing
    if ($code === null) {
      $code = 'PRJ-' . date('ymd') . '-' . substr((string)microtime(true), -4);
    }
    // INSERT
    $sql = 'INSERT INTO plt_projects (code, name, scope, start_date, deadline_date, status, owner_name)
            VALUES (?, ?, ?, ?, ?, ?, ?)';
    $pdo->prepare($sql)->execute([$code, $name, $scope, $start, $dead, $status, $owner]);
    $id = (int)$pdo->lastInsertId();
  }

  // ---- Milestones (overwrite strategy) ----
  ...
  if ($rows) {
    ...
    foreach ($rows as $m) {
      ...
      if (!in_array($mstat, ['pending','ongoing','done','delayed'], true)) $mstat = 'pending';
  ...
  $pdo->commit();
  echo json_encode(['ok' => 1, 'id' => $id, 'code' => $code], JSON_UNESCAPED_UNICODE);
