...
$p = [];
if ($from !== "") {
    $w[] = "s.schedule_date >= :from";
    $p[":from"] = $from;
}
if ($to !== "") {
    $w[] = "s.schedule_date <= :to";
    $p[":to"] = $to;
}
if ($status !== "") {
    $w[] = "s.status = :status";
...
$sqlTotals =
  ...
  SELECT
    ...
    ($hasDeliveredAt ? "s.delivered_at" : "s.eta_date") .
    ", s.eta_date), s.schedule_date)) AS avg_transit_days
  FROM plt_shipments s
  $where
";
$tot = $pdo->prepare($sqlTotals);
$tot->execute($p);
$totals = $tot->fetch(PDO::FETCH_ASSOC) ?: [
    "total" => 0,
    "delivered" => 0,
...
$totals["avg_transit_days"] = is_null($totals["avg_transit_days"])
    ? null
    : round((float) $totals["avg_transit_days"], 1);
...
$ln = $pdo->prepare($sqlLane);
$ln->execute($p);
$lanes = $ln->fetchAll(PDO::FETCH_ASSOC) ?: [];

/* Late (not delivered, past ETA) */
$sqlLate =
    "SELECT s.id, COALESCE(NULLIF(s.shipment_no,''), CONCAT('SHP-',s.id)) AS ref_no,
...
if ($fmt === "csv") {
    ...
    header(
        "Content-Disposition: attachment; filename=plt_shipments_report.csv"
    );
