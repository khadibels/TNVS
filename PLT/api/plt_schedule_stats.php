...
try {
    ...
    $stmt->execute();
    $todayCnt = (int) $stmt->fetchColumn();

    // Tomorrow
    $stmt = $pdo->prepare(
        "SELECT COUNT(*) FROM plt_shipments WHERE schedule_date = DATE_ADD(CURDATE(), INTERVAL 1 DAY)"
    );
    $stmt->execute();
    $tomorrowCnt = (int) $stmt->fetchColumn();

    // This week (Mon..Sun)
    $stmt = $pdo->prepare(
        "SELECT COUNT(*) FROM plt_shipments WHERE schedule_date BETWEEN :mon AND :sun"
    );
    $stmt->execute([
