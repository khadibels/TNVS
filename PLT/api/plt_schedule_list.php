...
try {
  ...
  $sort   = in_array(($_GET['sort'] ?? 'schedule'), ['schedule','eta','newest'], true) ? $_GET['sort'] : 'schedule';
  ...
  if ($status !== '') { $where[] = 's.status = :status'; $bind[':status'] = $status; }
  if ($search !== '') {
    $where[] = '(s.shipment_no LIKE :q OR p.name LIKE :q OR s.origin LIKE :q OR s.destination LIKE :q)';
    $bind[':q'] = '%'.$search.'%';
  }
  if ($from !== '') { $where[] = 's.schedule_date >= :from'; $bind[':from'] = $from; }
  if ($to   !== '') { $where[] = 's.schedule_date <= :to';   $bind[':to']   = $to; }
