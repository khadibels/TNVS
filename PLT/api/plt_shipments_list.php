...
try{
  if(!isset($pdo)) bad('DB missing',500);
  ...
  $where=[]; $args=[];
  if($search!==''){
    $where[]="(s.shipment_no LIKE ? OR s.origin LIKE ? OR s.destination LIKE ? OR s.vehicle LIKE ? OR s.driver LIKE ? OR p.name LIKE ?)";
  ...
  $st=$pdo->prepare("SELECT COUNT(*) FROM plt_shipments s LEFT JOIN plt_projects p ON p.id=s.project_id $whereSql");
