...
function bad(string $m, int $c = 400)
{
...
try {
    ...
    if ($search !== "") {
        $where[] =
            "(s.shipment_no LIKE ? OR s.origin LIKE ? OR s.destination LIKE ? OR s.vehicle LIKE ? OR s.driver LIKE ? OR p.name LIKE ?)";
        for ($i = 0; $i < 6; $i++) {
            $args[] = "%" . $search . "%";
        }
    }
    ...
    if ($sort === "eta") {
        $order = "ORDER BY s.eta_date ASC, s.id DESC";
    }
    if ($sort === "schedule") {
        $order = "ORDER BY s.schedule_date ASC, s.id DESC";
    }

    $st = $pdo->prepare(
        "SELECT COUNT(*) FROM plt_shipments s LEFT JOIN plt_projects p ON p.id=s.project_id $whereSql"
    );
    $st->execute($args);
